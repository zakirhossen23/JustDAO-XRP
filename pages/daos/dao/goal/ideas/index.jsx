...
export default function GrantIdeas() {
		...
		try {
			if (contract && id) {
				...
				for (let i = 0; i < Allvotes.length; i++) {
					...
					if (element === signerAddress.toLocaleUpperCase()) isvoted = true;
				}
	...
	async function VoteIdees() {
		...
		try {
			await sendTransaction(await window.contract.create_goal_ideas_vote(Number(Goalid), Number(id), signerAddress.toLocaleUpperCase() ));
		} catch (error) {
			console.error(error);
			return;
		}
	...
	async function PostComment(e) {
		...
		let newComment = {
			address: signerAddress.toLocaleUpperCase().toString(),
			message: Comment,
	...
	async function saveMessage(newComment) {
		await sendTransaction(await window.contract.sendMsg(Number(ideaId), JSON.stringify(newComment),signerAddress.toLocaleUpperCase()));
		removeElementFromArrayBYID(emptydata, 0, setemptydata)
	...
	async function sendReply(replyText, MessageId, MessageIndex) {
		...
		await sendTransaction(await window.contract.sendReply(Number(MessageId), JSON.stringify(newReply),Number(ideaId),signerAddress.toLocaleUpperCase()));
		removeElementFromArrayBYID(emptydata, 0, setemptydata)
